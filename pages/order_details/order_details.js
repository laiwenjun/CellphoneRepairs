...
Page({
  ...
  onLoad: function(a) {
    ...
    wx.getStorage({
      key: "azooo_userID",
      success: function(a) {
        console.log("请求详细订单信息~  a = ", a.orderid)
        ...
        var rspDetailedOrder = onfire.on('rspDetailedOrder', function(msg) {
          // 当消息被传递时，做具体的事
          var rsp = JSON.parse(msg.param)
          console.log("接到rspDetailedOrder事件----------------！！~~", rsp)
  ...
  applyInvoice: function(e) {
    var t = e.target.dataset.url,
      a = e.target.dataset.orderid,
      o = e.target.dataset.name,
      n = e.target.dataset.phone,
      s = e.target.dataset.price;
    wx.navigateTo({
  ...
  applyInvoiceSubmit: function(a) {
    var o = this,
      n = a.target.dataset.orderid,
      s = (a.target.dataset.masterid, this.data.apply_invoice_header),
      i = this.data.apply_invoice_address;
  ...
  cancelYes: function(a) {
    ...
    var rspCancelOrder = onfire.on('rspCancelOrder',function(msg){

      // 当消息被传递时，做具体的事
      var rsp = JSON.parse(msg.param)
      console.log("接到rspCancelOrder事件----------------！！~~", rsp)
      0 == rsp.error_code ? wx.showModal({
          title: "提示",
          content: "取消订单成功",
          showCancel: !1,
          success: function(e) {
