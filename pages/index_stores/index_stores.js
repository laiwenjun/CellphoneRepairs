...
Page({
    ...
    onLoad: function(r) {
        var n = this, s = decodeURIComponent(r.scene) || null, i = s.split("_")[0] || null, d = parseInt(s.split("_")[1]) || null;
        if (console.log("门店ID", d), "storeID" == i) wx.setStorage({
            key: "code_name",
            data: i
        }), wx.setStorage({
            key: "code_id",
            ...
            url: a.globalData.serverUrl + "getStoreName",
            data: {
                id: d
            },
            method: "post",
            ...
            if ("masterID" != i) return wx.showModal({
                title: "提示",
                content: "二维码出错！",
                showCancel: !1,
                success: function(a) {
            ...
            wx.setStorage({
                ...
                url: a.globalData.serverUrl + "getMaster",
                data: {
                    masterId: d
                },
                method: "post",
                success: function(a) {
                    wx.setNavigationBarTitle({
                        title: a.data.data.info.name + "（师傅）"
                    }), console.log("师傅ID", a.data.data.info);
                }
            });
        }
        wx.request({
        ...
        try {
            var c = wx.getStorageSync("Allbrandmodel"), u = wx.getStorageSync("AllFault");
            if (console.log("本地缓存：", c), c && u) {
                t = c.brandList, e = c.modelList, o = u.lagerRepairList, l = u.repairDetailList;
                var m = n.selectBrand(t[0].brandID);
                n.setData({
                    brandArr: t,
                    modelArr: m,
                    "dataForm.brandID": t[0].brandID,
                    "dataForm.brandName": t[0].brandName,
                    "dataForm.modelID": m[0].modelID,
                    "dataForm.color_name": m[0].color_name,
                    "dataForm.modelName": m[0].modelName,
                    "dataForm.thumbImg": m[0].thumbImg
                }), n.pipei(), n.setData({
        ...
        if (wx.getUpdateManager) {
            ...
            g.onCheckForUpdate(function(a) {
                console.log(a.hasUpdate);
            }), g.onUpdateReady(function() {
                wx.showModal({
                    title: "更新提示",
                    content: "更新到最新版小程序，获得更好体验。",
                    success: function(a) {
                        a.confirm && g.applyUpdate();
                    }
                });
        ...
        try {
            ...
            for (var n in e) if (e[n].smallModelName) {
                ...
                for (var s in r) if (r[s] == l || r[s] == l.split("<")[0] || r[s] == l.split("(")[0]) {
                    ...
                    for (var _ in D) D[_].modelID == e[n].modelID && (h = _);
                    o.setData({
                        brandValue: [ f ],
                        modelValue: [ h ],
                        modelArr: D,
    ...
    nextFun: function() {
        ...
        wx.setStorage({
            ...
            success: function() {
                wx.navigateTo({
                    url: "../scheme_stores/scheme_stores"
                });
            },
            fail: function() {
                wx.showModal({
